...
func (k *containerSyncer) Sync(ctx context.Context, s *Item) error {
	start := time.Now()

	if len(s.Copy) > 0 {
		logrus.Infoln("Copying files:", s.Copy, "to", s.Image)

		if err := PerformDockerComposeSync(ctx, s.Image, s.Copy, k.copyFileFn); err != nil {
			return errors.Wrap(err, "copying files")
		}
	}
	...
	if len(s.Delete) > 0 {
		...
		if err := PerformDockerComposeSync(ctx, s.Image, s.Delete, k.deleteFileFn); err != nil {
			return errors.Wrap(err, "deleting files")
		}
	}
